<?php
/**
 * Created by PhpStorm.
 * User: gmarten
 * Date: 28/08/15
 * Time: 23:15
 */
?>
<div class="section" id="content">
  <div class="container-fluid" style="background-image: url(/assets/img/container-bg.png?922006);" id="content-wrapper">
    <div class="row-fluid">
      <div class="col-md-12">
        <div class="top-divider" style="background-image: url(/assets/img/divider-star.png);"></div>
      </div>
    </div>
    <div class="row-fluid">
      <div class="col-md-4">
        <img data-id="img_main_left" src="/assets/img/img_main_left.png" class="img-responsive imagedropshadow imageUploadFilename" id="img_main_left">
      </div>
      <div class="col-md-4">
        <img data-id="img_main_center" src="/assets/img/img_main_center.png" class="img-responsive imagedropshadow imageUploadFilename" id="img_main_center">
      </div>
      <div class="col-md-4">
        <img data-id="img_main_right" src="/assets/img/img_main_right.png" class="img-responsive imagedropshadow imageUploadFilename" id="img_main_right">
      </div>
    </div>
    <div class="row-fluid">
      <div class="col-md-12">
        <div class="top-divider" style="background-image: url(/assets/img/divider-star.png);"></div>
      </div>
    </div>
    <div class="row-fluid">
      <div class="col-md-12 text-right">
        <img src="/assets/img/edit.png" class="icon textEditUpdate" data-id="paragraph_main_1" id="text1">
      </div>
      <div class="col-md-12">
        <p id="content_paragraph_main_1" class="paragraph"></p>
      </div>
    </div>
  </div>
</div>
